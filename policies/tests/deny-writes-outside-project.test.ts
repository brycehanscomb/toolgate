...
const run = adaptHandler(denyWritesOutsideProject.action!, denyWritesOutsideProject.handler as any);
...
describe("deny-writes-outside-project", () => {
  describe("allows writes within project root", () => {
    it("allows Write to file in project", async () => {
      const result = await run(write("/home/user/project/src/foo.ts"));
      expect(result.verdict).toBe(NEXT);
    });

    it("allows Edit to file in project", async () => {
      const result = await run(edit("/home/user/project/src/foo.ts"));
      expect(result.verdict).toBe(NEXT);
    });

    it("allows nested paths", async () => {
      const result = await run(write("/home/user/project/a/b/c/d.ts"));
      expect(result.verdict).toBe(NEXT);
    });
  });
  ...
  describe("denies writes outside project root", () => {
    it("denies Write to /etc/passwd", async () => {
      const result = await run(write("/etc/passwd"));
      expect(result.verdict).toBe(DENY);
    });

    it("denies Edit to home directory file", async () => {
      const result = await run(edit("/home/user/.bashrc"));
      expect(result.verdict).toBe(DENY);
    });

    it("denies Write to sibling directory", async () => {
      const result = await run(write("/home/user/other-project/foo.ts"));
      expect(result.verdict).toBe(DENY);
    });
  });
  ...
  describe("handles path traversal tricks", () => {
    it("denies path that is a prefix but not a subdirectory", async () => {
      const result = await run(write("/home/user/project-evil/foo.ts"));
      expect(result.verdict).toBe(DENY);
    });
  });
  ...
  describe("passes through when no project root", () => {
    it("passes through Write with no projectRoot", async () => {
      const result = await run(write("/etc/passwd", null));
      expect(result.verdict).toBe(NEXT);
    });
  });
  ...
  describe("denies Bash redirects outside project", () => {
    ...
    it("denies cat >> /outside/path (append)", async () => {
      const result = await run(bash("cat >> /home/user/.bashrc"));
      expect(result.verdict).toBe(DENY);
    });
    ...
    it("denies tee writing outside project", async () => {
      const result = await run(bash("echo hi | tee /etc/evil"));
      expect(result.verdict).toBe(DENY);
    });
    ...
    it("allows Bash with no projectRoot", async () => {
      const result = await run(bash("cat > /etc/passwd", null));
      expect(result.verdict).toBe(NEXT);
    });
    ...
    it("denies redirect to tilde path outside project", async () => {
      const result = await run(bash("echo hi > ~/.bashrc"));
      expect(result.verdict).toBe(DENY);
    });

    it("denies tee with tilde path", async () => {
      const result = await run(bash("echo hi | tee ~/evil.txt"));
      expect(result.verdict).toBe(DENY);
    });

    it("denies redirect with relative path outside project", async () => {
      const result = await run(bash("echo hi > ../other-project/foo.ts"));
      expect(result.verdict).toBe(DENY);
    });

    it("allows redirect with relative path inside project", async () => {
      const result = await run(bash("echo hi > ./src/foo.ts"));
      expect(result.verdict).toBe(NEXT);
    });
  });
  ...
  describe("denies write commands (cp/mv/install) outside project", () => {
    it("denies cp to /tmp", async () => {
      const result = await run(bash("cp src/file.ts /tmp/file.ts"));
      expect(result.verdict).toBe(DENY);
      expect((result as any).reason).toContain("Use ./tmp/ within your project instead");
    });
    ...
    it("denies mv to outside project", async () => {
      const result = await run(bash("mv src/old.ts /home/user/other/old.ts"));
      expect(result.verdict).toBe(DENY);
    });

    it("denies install to outside project", async () => {
      const result = await run(bash("install -D src/bin /usr/local/bin/tool"));
      expect(result.verdict).toBe(DENY);
    });

    it("denies cp to tilde path", async () => {
      const result = await run(bash("cp src/file.ts ~/backup.ts"));
      expect(result.verdict).toBe(DENY);
    });

    it("allows cp within project", async () => {
      const result = await run(bash("cp src/a.ts src/b.ts"));
      expect(result.verdict).toBe(NEXT);
    });

    it("allows cp to relative path in project", async () => {
      const result = await run(bash("cp src/a.ts ./tmp/backup.ts"));
      expect(result.verdict).toBe(NEXT);
    });

    it("passes through cp with no projectRoot", async () => {
      const result = await run(bash("cp src/a.ts /tmp/a.ts", null));
      expect(result.verdict).toBe(NEXT);
    });
  });
  ...
  describe("safe write targets pass through", () => {
    it("passes through redirect to /dev/null", async () => {
      const result = await run(bash("cat foo 2>/dev/null"));
      expect(result.verdict).toBe(NEXT);
    });

    it("passes through redirect to /dev/stderr", async () => {
      const result = await run(bash("echo err > /dev/stderr"));
      expect(result.verdict).toBe(NEXT);
    });

    it("passes through redirect to /dev/stdout", async () => {
      const result = await run(bash("echo ok > /dev/stdout"));
      expect(result.verdict).toBe(NEXT);
    });

    it("passes through tee to /dev/null", async () => {
      const result = await run(bash("echo hi | tee /dev/null"));
      expect(result.verdict).toBe(NEXT);
    });
